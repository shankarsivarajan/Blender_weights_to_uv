...
class Vertex_Weights_to_UV_Active(bpy.types.Operator):
    """Convert active vertex group to UV Map"""
    bl_idname = 'object.vert_weights_to_uv_active'
...
class Vertex_Weights_to_UV_All(bpy.types.Operator):
    """Convert all vertex groups to UV Map (currently limited to 8 maps)"""
    bl_idname = 'object.vert_weights_to_uv_all'
    bl_label = 'All Vertex Groups'
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        for vertex_group in bpy.context.object.vertex_groups:
            current_group = vertex_group.name
            print(current_group)
            
            weight = []
            for v in me.vertices:
                try:
                    weight.append(ob.vertex_groups[current_group].weight(v.index))
                except:
                    weight.append(0)
            
            weight = np.array(weight)        
            
            uv_layer_name = current_group
...
class WeightsToUV_Menu(bpy.types.Menu):
    ...
    def draw(self, context):
        self.layout.operator(Vertex_Weights_to_UV_Active.bl_idname)
        self.layout.operator(Vertex_Weights_to_UV_All.bl_idname)
...
def register():
    ...
    bpy.utils.register_class(Vertex_Weights_to_UV_Active)
    bpy.utils.register_class(Vertex_Weights_to_UV_All)
...
def unregister():
    ...
    bpy.utils.unregister_class(Vertex_Weights_to_UV_Active)
    bpy.utils.unregister_class(Vertex_Weights_to_UV_All)
